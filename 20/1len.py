...
def solve(filepath):
    ...
    while True:
        ...
        if char == '$':
            ...
            current_dist = grid[player_location[1]][player_location[0]].distance
            diff = current_dist - last_start_dist
            if not map_index in max_dist_map:
                max_dist_map[map_index] = diff
            elif diff > max_dist_map[map_index]:
                max_dist_map[map_index] = diff
            ...
            if index in max_dist_map:
                print("short circuit")
                total = max_dist_map[index] + grid[player_location[1]][player_location[0]].distance
                if total > max_dist:
                    max_dist = total
                if len(branch_stack) == 0:
                    break
