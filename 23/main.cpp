...
int main()
{
	...
	while(bounding.r >= 1)
	{
		...
		// binary search for next radius that actually changes count of intersecting
		...
		// These next 3 loops just speed up the homing in process
		for(const auto direction : directions())
		{
			auto next_bounding = bounding;
	...
	std::cout << "Answer: " << std::abs(bounding.x)+std::abs(bounding.y)+std::abs(bounding.z) << std::endl;
